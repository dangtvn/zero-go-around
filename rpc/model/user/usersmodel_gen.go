...
var (
	...
	usersRowsExpectAutoSet   = strings.Join(stringx.Remove(usersFieldNames, "`id`", "`update_time`", "`create_at`", "`created_at`", "`create_time`", "`update_at`", "`updated_at`"), ",")
	usersRowsWithPlaceHolder = strings.Join(stringx.Remove(usersFieldNames, "`id`", "`update_time`", "`create_at`", "`created_at`", "`create_time`", "`update_at`", "`updated_at`"), "=?,") + "=?"
...
type (
	...
	Users struct {
		Id        int64          `db:"id"`
		Name      sql.NullString `db:"name"`       // name
		Status    sql.NullInt64  `db:"status"`     // status
		CreatedAt sql.NullTime   `db:"created_at"` // created time
	}
)
...
func (m *defaultUsersModel) Insert(ctx context.Context, data *Users) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.Name, data.Status)
	}, usersIdKey)
	return ret, err
}
...
func (m *defaultUsersModel) Update(ctx context.Context, data *Users) error {
	...
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.Name, data.Status, data.Id)
	}, usersIdKey)
	return err
}
